...
class ChatConcateUtilization(BaseUtilization):
    """
    Concate memory pieces into a string for application.
    """
    def __init__(self, config):
    ...
    def concate_list(self, l):
        if self.config.list_config.index:
            l = ['[turn %d]%s %s' % (int(index // 2) + 1,'User:' if index % 2 == 0 else "System:", m) for index, m in enumerate(l)]
        if len(l) == 0:
            return 'None'
        return self.config.list_config.sep.join(l)
...
class ActionConcateUtilization(BaseUtilization):
    """
    Concate memory pieces into a string for application.
    """
    def __init__(self, config):
    ...
    def concate_list(self, l):
        if self.config.list_config.index:
            l = ['[turn %d] %s' % (int(index + 1), m) for index, m in enumerate(l)]
        if len(l) == 0:
            return 'None'
        return self.config.list_config.sep.join(l)
