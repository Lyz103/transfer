...
from Memory.Recall import ActionMemoryRecall
from Memory.Store  import ActionMemoryStore
from Memory.utils.Storage import LinearStorage

class ActionMemory(ExplicitMemory):
    """
    Action Memory to save User's past action.
    """
    def __init__(self, config) -> None:
        super().__init__(config)

        self.storage = LinearStorage(self.config.args.storage)
        self.store_op = ActionMemoryStore(self.config.args.store, storage = self.storage)
        self.recall_op = ActionMemoryRecall(self.config.args.recall, storage = self.storage)
...
if __name__ == "__main__":
    ...
    def ActionMemory1():
        ...
        memory = ActionMemory(memory_config)
