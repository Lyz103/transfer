...
class Evaluation:
    ...
    def interact(self):
        keys = ['formality', 'conciseness', 'sentiment', 'proactiveness', 'carelessness', 'emojis']

        for user in self.user_agent:
            for crs in self.crs_agent:
                for key in keys:
                    LOGGER.info(f"Start interaction between {user.name} and {crs.name}_{key}")
                    # create the save path
                    path = os.path.join(self.save_path, f"{user.name}_{crs.name}_{key}")
                    if not os.path.exists(path):
                        os.makedirs(path)

                    for i in range(self.interaction_num):
                        try:
                            user.refresh_profile(i, key)
    ...
    def compare(self):
        for user in self.user_agent:
            ...
            for crs in self.crs_agent:
                ...
                # compare the user
                with tqdm(total=(min(self.compare_times, min(len(history_list_main), len(history_list_compare))))) as pbar:
                    for compare_times in range(min(self.compare_times, min(len(history_list_main), len(history_list_compare)))):
                        # 随机选择一个对话
                        # h_main = random.choice(history_list_main)
                        # h_compare = random.choice(history_list_compare)
                        h_main = history_list_main[compare_times]
                        h_compare = history_list_compare[compare_times]
                        main_dialogue = {}
                        compare_dialogue = {}
                        ...
                        for key in result_1.keys():
                            if result_1[key] == result_2[key]:
                                ...
                                result.update({key: -1})
                        print("h_main:", h_main)
                        self.save_result(result={"history_id": compare_times,
                                                 "compared_history_id": compare_times,
                                                 "compare_result": result,"h_main": h_main,"h_compare": h_compare},
