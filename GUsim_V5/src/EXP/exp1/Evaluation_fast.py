...
class Evaluation:
    ...
    def interact(self):
        ...
        def process_single_interaction(user, crs, i, path):
            try:
                ...
                LOGGER.error(f"Error in interaction {i + 1}: {e}")
        ...
        for user in self.user_agent:
            for crs in self.crs_agent:
                ...
                # Use ThreadPoolExecutor to handle multiple interactions in parallel
                with ThreadPoolExecutor(max_workers=8) as executor:
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            if not result:
                                LOGGER.warning("An interaction failed to complete successfully")
                        except Exception as e:
                            LOGGER.error(f"Error processing an interaction: {e}")
    ...
    def evaluates(self, metric, h, result_path_raw):
                ...
                for metric in self.metrics:
                    ...
                    with ThreadPoolExecutor(max_workers=8) as executor:
                        ...
                        for future in futures:
                            ...
                            now_counter.update_dict(future.result().counter)
    ...
    def compares(self, history_list_main, history_list_compare, result_path_raw):
        print("---------")
        # 随机选择一个对话
                ...
                with ThreadPoolExecutor(max_workers=8) as executor:
                    futures = [
                        executor.submit(
                            self.compares,
                            history_list_main,
                            history_list_compare,
                            result_path_raw
                        )
                        for _ in range(self.compare_times)
                    ]
                    for future in futures:
                        future.result()  # Ensure each task is completed
                        print("Finish one compare")
                self.save_total_result(save_path_raw=result_path_raw, save_path=result_path)
